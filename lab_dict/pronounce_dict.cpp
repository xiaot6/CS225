...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto look1 = dict.find(w1);
    auto look2 = dict.find(w2);
    if (look1 ==dict.end() || look2 == dict.end() ) {
      return false;
    }
    if(look1->second.size() != look2->second.size()){
      return false;
    }
    for(size_t i = 0; i < look1->second.size(); i++){
        if(look1->second[i] != look2->second[i]){
          return false;
        }
      }
    return true;
