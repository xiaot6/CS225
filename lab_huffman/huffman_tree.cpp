...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while(singleQueue.size() + mergeQueue.size() !=1) {
       ...
       int newfren = tmp1->freq.getFrequency() + tmp2->freq.getFrequency();
       TreeNode * tomerge = new TreeNode(newfren);
       tomerge->left = tmp1;
       tomerge->right = tmp2;
       mergeQueue.push(tomerge);
     }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     if (!bfile.hasBits()) {
      ...
      if (bfile.getNextBit()) {
      		Frequency newfrequency(bfile.getNextByte(), 0);
      		TreeNode *toreturn = new TreeNode(newfrequency);
          return toreturn;
      } else {
