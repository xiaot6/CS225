...
int DHHashTable<K, V>::findIndex(const K& key) const
{
   ...
   unsigned int idx = hashes::hash(key, size);
    unsigned int idx2 = hashes::secondary_hash(key, size);
...
template <class K, class V>  // don't delete elements since we just moved their pointers aroundass V>
...
void DHHashTable<K, V>::resizeTable()
{
    ...
    for (size_t slot = 0; slot < size; slot++) {
        if (table[slot] != NULL) {
            ...
            size_t idx2 = hashes::secondary_hash(table[slot]->first, newSize);
            size_t i = 0;
            size_t idx = h;
            while (temp[idx] != NULL)
            {
                ++i;
                idx = (h + idx2*i) % newSize;
            }
            temp[idx] = table[slot];
            should_probe[idx] = true;
        }
