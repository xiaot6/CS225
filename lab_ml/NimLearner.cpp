...
NimLearner::NimLearner(unsigned startingTokens) : g_(true, true) {
    ...
    for(unsigned i = 0; i <= startingTokens; i++){
      ...
      g_.insertEdge("p1-"+to_string(1),"p2-0");
      g_.insertEdge("p2-"+to_string(1),"p1-0");
      g_.setEdgeWeight("p1-"+to_string(1),"p2-0", 0);
      g_.setEdgeWeight("p2-"+to_string(1),"p1-0", 0);
...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 /* Your code goes here! */
  ...
  while(g_.getAdjacent(curr).size() != 0){
    std::vector<Vertex> vector = g_.getAdjacent(curr);
    int randomnumber = rand() % vector.size();
    temp = vector[randomnumber];
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 /* Your code goes here! */
  ...
  for(int i = path.size() -1; i > -1;i-- ){
    ...
    g_.setEdgeWeight(edge.source, edge.dest, oldWeight + flag);
    flag *= -1;
  }
